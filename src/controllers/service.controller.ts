...
const storeService = async (req: Request, res: Response) => {
	const { categoryId, price, experience, description, lat, lng } = req.body;
	...
	try {
		...
		return res
			.status(201)
			.json({ message: 'Service added successfully', data: response });
	} catch (error) {
...
const fetchServicesNearYou = async (req: Request, res: Response) => {
	...
	try {
		// Find all user's services
		const services = await servicesNearYou(
			latitude,
			longitude,
			paginateData.offset,
			paginateData.limit,
...
const suggestedServicesForUser = async (req: Request, res: Response) => {
	...
	try {
		...
		const services = await suggestedServices(
			category,
			paginateData.offset,
			paginateData.limit,
			p,
