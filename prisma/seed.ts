...
async function main() {
	const dogWalking = await prisma.category.findFirst({
		where: { name: 'Dog Grooming' },
	});
	if (dogWalking) {
		for (let i = 0; i < 20; i++) {
			// User information
			...
			const imageKey: string = imageUrl.replace(
			...
			const fileUrl: string = faker.image.urlLoremFlickr({
				category: 'animals',
			});
